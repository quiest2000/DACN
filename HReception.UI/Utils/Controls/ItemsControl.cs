...
using Xamarin.Forms;
namespace HReception.UI.Utils.Controls
{
    public class ItemsControl : ContentView
    {
        ...
        public static readonly BindableProperty ItemsPanelProperty = BindableProperty.Create(
            nameof(ItemsPanel), typeof(Layout<View>), typeof(ItemsControl),
            propertyChanged: (bindable, oldValue, newValue) => ((ItemsControl)bindable).OnItemsPanelPropertyChanged((Layout<View>)oldValue, (Layout<View>)newValue));
        ...
        {
            if (stack != null && !oldValue.Equals(newValue))
            {
        ...
        void FillContainer(Layout<View> container, IList newValue, DataTemplate itemTemplate)
        {
            if (container == null)
            {
